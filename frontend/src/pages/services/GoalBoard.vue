...
const getSavedGoals = () => {
  const saved = localStorage.getItem('qulay_goals')
  if (saved) return JSON.parse(saved)
  return [
    { id: 1, title: 'IELTS dan 7.5 olish', category: 'O\'qish', progress: 65, date: '2026-12-01', history: [] },
    { id: 2, title: 'Yangi mashina uchun jamg\'arma', category: 'Moliya', progress: 40, date: '2027-05-01', history: [] },
  ]
}

const goals = ref(getSavedGoals())

watch(goals, (newVal) => {
  localStorage.setItem('qulay_goals', JSON.stringify(newVal))
}, { deep: true })
...
function logProgress(g) {
  if (!g.history) g.history = []
  g.history.unshift({ date: new Date().toISOString().slice(0,10), action: `Progress o'zgardi: ${g.progress}%` })
}
</script>

<template>
  <ToolShell icon="🎯" title="Maqsadlar doskasi" description="Katta hayotiy va professional maqsadlarni belgilang hamda rivojlanishni kuzating" hint="Maqsad qo'shing va har safar natijaga erishganingizda progress foizini oshirib boring. Barcha ma'lumotlar xotirada saqlanadi.">
      ...
      <div class="space-y-4">
        <div v-for="g in goals" :key="g.id" class="p-4 rounded-xl bg-white dark:bg-slate-800 border border-slate-200/80 dark:border-slate-700 shadow-sm space-y-3">
          ...
          <div>
            ...
            <input v-model.number="g.progress" @change="logProgress(g)" type="range" min="0" max="100" step="5" class="w-full accent-brand-600" />
          </div>
          ...
          <div v-if="g.history && g.history.length > 0" class="mt-2 text-[10px] text-slate-400">
            <p class="font-semibold mb-1">Tarix:</p>
            <ul class="space-y-0.5 pl-2">
              <li v-for="(h, idx) in g.history.slice(0,3)" :key="idx">- {{ h.date }}: {{ h.action }}</li>
            </ul>
          </div>
        </div>
      </div>
